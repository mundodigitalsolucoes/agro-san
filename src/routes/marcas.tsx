...
function MarcasPage() {
  return (
    <>
      <PageHero
        eyebrow="Marcas"
        title="Um ecossistema de marcas conectadas ao produtor."
        subtitle="Cada marca do grupo tem um papel claro e complementar, do institucional ao pet."
      />
      <section className="mx-auto max-w-6xl px-6 lg:px-10 py-20 space-y-8">
        {MARCAS.map((m, i) => (
          ...
          >
            <div
              className={`h-64 lg:h-full min-h-[240px] bg-gradient-to-br ${m.tint} grid place-items-center p-8`}
            >
              <img
                src={m.logo}
                alt={`Logo ${m.name}`}
                className="max-h-40 max-w-full object-contain drop-shadow-xl transition-transform duration-500 group-hover:scale-105"
                loading="lazy"
              />
            </div>
            <div className="p-8 lg:pr-12">
              ...
              <h2 className="mt-2 font-display text-3xl font-bold text-[color:var(--forest-deep)]">
                {m.name}
              </h2>
              ...
              <ul className="mt-5 grid sm:grid-cols-3 gap-2">
                {m.bullets.map((b) => (
                  <li
                    key={b}
                    className="text-sm text-[color:var(--foreground)] flex items-center gap-2"
                  >
              ...
              {m.href.includes("?") ? (
                <a
                  href={m.href}
                  className="mt-7 inline-flex items-center gap-2 rounded-full bg-gradient-forest text-white px-6 py-3 text-sm font-semibold shadow-soft hover:shadow-elegant transition-all"
                >
                  {m.cta}

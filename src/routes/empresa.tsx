...
export const Route = createFileRoute("/empresa")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Conheça a Agro'San Linea: quem somos, missão, valores e os cinco pilares que sustentam nossa atuação no agronegócio nacional desde 2012." },
    ...
    <>
      ...
      <section className="mx-auto max-w-5xl px-6 lg:px-10 py-20 space-y-10">
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { t: "Missão", d: "Facilitar a negociação no agronegócio nacional com produtos, tecnologia e atendimento consultivo." },
            { t: "Visão", d: "Ser referência em soluções integradas para o produtor rural, do Norte ao Sul do Brasil." },
            { t: "Valores", d: "Ética, responsabilidade, inovação, proximidade com o produtor e compromisso com o campo." },
          ].map((v) => (
            <div key={v.t} className="rounded-2xl bg-white p-6 border border-[color:var(--border)] shadow-soft">
              <div className="text-xs uppercase tracking-wider text-[color:var(--gold)] font-semibold">{v.t}</div>
        ...
        <div className="rounded-3xl bg-gradient-forest text-white p-10 lg:p-14 shadow-elegant">
          <div className="text-xs uppercase tracking-[0.25em] text-[color:var(--gold-soft)] font-semibold">Nossos pilares</div>
          <h2 className="mt-3 text-3xl font-display font-bold">Cinco frentes, um único propósito.</h2>
          <div className="mt-8 grid grid-cols-2 md:grid-cols-5 gap-4">
            {PILARES.map((p) => (
              <div key={p.title} className="rounded-2xl bg-white/10 border border-white/15 p-5 backdrop-blur-sm">
        ...
        <div className="text-center">
          <Link to="/nossa-historia" className="inline-flex items-center gap-2 rounded-full bg-gradient-forest text-white px-6 py-3 font-semibold shadow-elegant">
...
export function PageHero({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle?: string }) {
  return (
    <section className="relative bg-gradient-to-br from-[color:var(--cream)] via-white to-[color:var(--accent)] py-20 lg:py-28">
      <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: "radial-gradient(circle at 20% 20%, var(--forest-deep) 1px, transparent 1px)", backgroundSize: "24px 24px" }} />
